...
//
// We will work with a model of the Staubli RX 90 BL robot
// Now we create kinematic data of a Staubli RX 90 BL manipulator
//
//##########################################################
close;clc;
L = list();
...
L(2) = Link([ 0       0      450        0     0 ]);
L(3) = Link([ 0       650      0       %pi/2    0 ]);
L(4) = Link([ 0       0       0        -%pi/2     0 ]);
L(5) = Link([ 0       85       0         %pi/2   0 ]);
L(6) = Link([ 0       0       0         0      0 ]);

qz = [0 -%pi/2 0 0 0 0]; // zero angles, L shaped pose
//Pose 0; At ZERO position
qready = [%pi/2 0 %pi/2 %pi/4 %pi/2 %pi/4];
// ready pose, arm up
//q0 =[0   0   0   0   -%pi/2   0];
mHP6=SerialLink(L, 'name', 'Staubli RX 90 BL');
//##########################################################
